...
HOW A ROUND IS SHAPED, AND HOW LONG IT COSTS (W487 — measured, so it is not re-argued each time):
  ...
  B2 RUN THE MACHINERY ONCE PER TREE, NOT PER FIX. The full suite is 46 minutes over 392 tests
     ...
     (-k) only. The full suite and the full blind sweep run on the FINAL tree — twice at most per
     round: once before the refutation pass and once on its fixes. W485 spent ~4h of machine time on
     three suites and four sweeps; once-per-tree is ~1h10 for the same evidence.
  B3 OVERLAP WHAT DOES NOT SHARE STATE. The refutation workflow runs in isolated worktrees, so it
     runs CONCURRENTLY with the full suite (≈45 min saved). Never two pytest runs at once, and never
     a refuter while a break harness is mutating the tree.
...
every register change (add · close · drop · reslot · route · done · render); never edit between the markers.
  Next: P1.18 The third truth pass — 35 follow-ups ride it.
  Then, in order (the follow-ups riding each): P2.1 0 · P2.2 0 · P2.3 1 · P2.4 32 · P2.5 0 · P2.6 13 · P2.7 1 ·
    P2.8 7 · P2.9 26 · P3.0 0 · P3.1 0 · P3.2 0 · P3.3 0 · P3.4 0 · P3.5 0 · P3.6 0 · P3.7 0 · P3.8 0 ·
    P3.9 0 · P3.10 0 · P3.11 0 · P3.12 5 · P3.13 0 · P3.14 1 · P3.15 2 · P3.16 1 · P3.17 5 · P3.18 1 ·
    P3.19 2 · P4.1 0 · P4.2 0 · P4.3 0 · P4.4 0 · P4.5 0 · P4.6 0
  Highest priority in P1.18 (score · area): FU-125 88.2 lifecycle · FU-098 81.8 lifecycle ·
    FU-100 81.8 lifecycle · FU-127 68.6 fabric · FU-112 63.6 domains_ux
  Done: 17 of 53 items — P1 17/18 · P2 0/9 · P3 0/20 · P4 0/6.
  Follow-up completion weighted by priority — P1: 63.2% of its rows' priority closed (68 of 103 rows); every phase's rows: 56.3% (the retired pre-plan queue left out).
  Follow-ups: 132 open — 132 ride a plan item (35 high), 0 unscheduled, 0 awaiting the Owner; 128 done, 4 dropped.
<!-- plannow:end -->
<!-- pace:begin (generated by scripts/followups.py render - never edit by hand) -->
WHERE THIS IS GOING (generated — 17 of 53 plan entries done, 132 open rows)
  PACE, over the last 6 round(s) that closed anything (W488, W489, W490, W491, W492, W493): 9.83 rows closed per round, 2.5 found, net 7.33.
  RATE USED: 9.83 rows/round — steady (one-time intakes excluded).
  NEXT — P1.18: 35 open rows ≈ 14 round(s) — at P1.18's OWN rate: 16 row(s) over the last 6 build round(s) = 2.67/round.
  ALL OPEN ROWS: 132 ≈ 14 round(s) at the overall rate, which covers every item's rows.
  BY ITEM (each at its OWN measured rate; — = too few rounds have closed one of its rows to measure): P1.18 35r≈14 · P2.3 1r— · P2.4 32r≈9 · P2.6 13r≈26 · P2.7 1r— · P2.8 7r≈9 · P2.9 26r≈16 · P3.12 5r—
  BIGGEST BATCH: C1 (certifies what it could not assess) — one mechanism closes 6 row(s) across 32 file(s), advancing 6 more. Items: P1.18, P2.4, P2.6, P2.9.
...
Open 132 (132 scheduled, 35 high · 0 unscheduled · 0 awaiting the Owner) · done 128 · dropped 4.
  P1.18 — The third truth pass
    ...
    FU-258 [high] [p 45.0] Most AI calls do not thread the account id, so their prompts and answers land in the SHARED memory namespace that the two recall surfaces read for every user — W490 set out to correct a false privacy line and found the claim under it is false too. memory.add_memory stamps owner_id or PLATFORM_NS (agentic_core/ai/memory.py:86) and gateway.query_meta/stream_meta write EVERY completion into it (gateway.py ~195 and ~340). Only a handful of the ~57 gateway call sites in agentic_core pass owner_id at all: board directives, VSB board packs, business-plan text, capital-fund analysis, forge stages, studio stages and the QEP surfaces all omit it, so their prompts AND answers land in the shared platform namespace. query_memory's allow-set is {owner_id or PLATFORM, PLATFORM} (memory.py:112), so the two surfaces that still use recall — the avatar and the AI-CEO chat — can retrieve platform-namespace content that belonged to a DIFFERENT user. W489 removed recall from generation, which closes the biggest consumer, but the WRITES continue and the two conversational readers remain. The Settings page now warns the user plainly rather than claiming their interactions are stored under their account. FIX: thread the authenticated owner_id through every gateway call that has one (a request-scoped default would be better than 57 edits — the same lesson as W489's augment default), and make a write with NO id an explicit decision rather than a fallback; then the warning can come down. Note for whoever takes it: decide with the Owner what happens to the rows already written to the shared namespace. (found W490 refutation (avatar-and-claims lens))
  ...
  P2.3 — Avatar + profile honesty
    FU-251 [medium] [p 6.2] The avatar is the one caller that keeps cross-request recall — its tenant scoping is asserted in a comment, not by an executing leg — W488 set augment=False on every generation-class query_meta caller in the repo and left exactly one deliberate exception: the avatar conversation (agentic_core/avatars/api.py), where recall is the point and W333 scopes it to owner_id. The exception is documented in place and a guard leg asserts the comment and the augment=True is there — but NOTHING executes the claim the comment makes: that an authenticated caller reaches only their own namespace plus platform memory, and an UNAUTHENTICATED caller (owner_id None) reaches platform memory only, never another tenant's. That is the one place in the repo where a cross-tenant leak would now be invisible, so it is the one place that needs a behavioural leg: seed two tenants' memories, ask the avatar as each, and prove neither sees the other's. (found W488 refutation (board-recall lens, follow-on))
  P2.4 — The scatter: 67 ops in 38 clusters, 3–4 per round, audit-before-wire, retire freely
    ...
    FU-247 [medium] [p 17.5] The server-side export prints the opposite provenance label: floor-served content ships as produced on the AI fabric — W485 gave every BROWSER export a provenance line (lib/api.ts provenanceLine, used by My Work and DomainTool). agentic_core/api/deliverables.py still stamps its own page one with wording that credits Workstation's AI fabric for content the deterministic floor composed, so the server-produced artefact says the opposite of what the browser-produced one now says about the same text. FIX: one statement for both — the export names what served it, and a floor-served export says the floor composed the headings and supplied no analysis. (found W485 refuters (verified real) — found while applying the rule, not delivered)
    FU-260 [medium] [p 15.9] Channels drawer collects notes but reaches no channel backend — packages/ui/src/CommandCenter.tsx handleSendQuery appends the text to local state and makes no request; W491 relabelled the control honestly (it now says the note is kept on screen and nothing answers) but the Channels feature itself is unwired. Either wire each channel to its backend or retire the input. (found W491 refuter (reach lens))
    ...
    FU-256 [medium] [p 11.2] The 29 generation callers that carried cross-request recall were never probed for what the recall was doing to their output — W489 flipped the gateway's augment default to False after finding that query/stream/stream_meta still inherited True — 29 generation-class callers, including the live nine-stage synthesis cascade, the CEO blueprint and the digital-twin modeller, had another request's content prepended and presented as analysis of their own subject. The flip stops it happening again, but nothing establishes what it was doing while it happened: no stored output is marked as possibly contaminated, and the memory pool those calls WROTE (gateway logs every interaction) still holds the blended text, so a future recall-enabled caller can retrieve it. FIX: (a) sample the persisted outputs of those 29 surfaces for text that plainly came from another request (the native floor's tell is a Domain: or Problem: label whose value belongs to a different subject) and count how many are affected; (b) decide with the Owner whether contaminated stored outputs are relabelled, regenerated or left with a disclosure; (c) check whether memory rows written from augmented prompts should be marked so recall never re-serves a blend as one tenant's own. (found W489 refutation (recall-default lens, follow-on))
    ...
    FU-249 [medium] [p 2.2] The 46-minute suite is the round's biggest cost, and xdist has never been tried against the shared-store risk — W487 measured it: integration_tests/ is 46 min over 392 tests (mean 7.1s; top forty = 69%; slowest 134s), and a round runs it once or twice. That is the single largest component of a round's wall clock. pytest-xdist could cut it 3-4x, but this repo's shared stores are exactly what makes it dangerous: [[reference-test-data-isolation-and-concurrency-corruption]] records ~40 false failures from two suites sharing memory.json and the UEG ledgers. FIX (to be tried deliberately, not blind): give each xdist worker its own DATA_DIR / WORKSTATION_DATA_DIR / WORKSTATION_UEG_PATH / PROJECTS_DIR via a worker-id fixture, then prove it by running the suite serially and in parallel and comparing the pass sets exactly. If any test cannot be isolated, mark it serial rather than weakening it. (found W487 measurement of where a round's time goes)
  ...
  P2.6 — The perimeter and the gate
    ...
    FU-263 [medium] [p 20.5] No surface calls the evolution apply endpoint, so an approved evolution can only be applied by API — W493's self-check found generation_advanced produced by apply_approved_evolution and read by nothing. The cause is that no page calls POST /api/v1/vsb/{id}/evolution/apply at all: the cockpit files proposals (Evolve) and the Owner approves the change record in Change Control, but nothing in the UI then applies it, so the genome mutation depends on an API call the Owner cannot make from the product. Either add an Apply control where the approval is visible (Change Control or the cockpit), or have the approval path itself trigger the apply and report generation_advanced. (found W493 self-check + refuter)
  ...
  P2.8 — Bespoke swarms
    ...
    FU-232 [medium] [p 8.0] The transformation cascade has no DELIVERY check, so a run can never be validated and the plan write-back is unreachable — W481 made the verdict honest: 'validated' requires a stage whose check measures a delivered outcome, and no stage has one (stages 1-4 presence, 5-6 nothing, 7 a governance decision, 8 an artifact write, 9 provenance). Every run returns validated=null and the §5 loop closure that moves a living-plan objective planned->in_progress never fires in production (both ways are covered by tests through the summarise_validation seam). FIX: give the cascade one real delivery check — a Build-to-Order order created, or a plan objective's measured progress moving — declared checks='delivery'. Until then the honest state is 'not assessable'. (found W481 refuters (verified real))
    FU-239 [medium] [p 6.8] No §11 framework can assess a subject, so the screen can only ever refuse or review — W483 made the rule explicit — only coverage 'engine' may carry a pass — and then showed that NOTHING in the screen currently earns it: sharia_halal and uk_legal are keyword/vocabulary screens, regulatory and ehs are keyword screens, the constitutional row is a nine-intent substring gate, and the ethical engine's three ethical dimensions are word lists (its fourth dimension measures delivery coverage, which is not ethics). So `assessed_by` is always empty and `compliant` is never True. That is now stated honestly rather than hidden, but the CAPABILITY gap is real: the platform has no instrument that can clear a subject in any of the six areas. FIX: give at least one framework a real assessor (the Halal officer engine is the nearest candidate — it already runs and returns structured violations), or state in the vision that §11 is a flagging layer only and nothing downstream may treat it as clearance. (found W483 (the §11 screen round) — found while applying the rule, not delivered)
    FU-240 [medium] [p 6.8] §4.5 candidate ranking is form-only again, and form saturates so candidates tie — With no §11 framework able to assess a candidate (see the row above), _screen_candidate returns compliance=None and safety=None for every candidate, so stage 5 ranks on form alone. W419 existed because form SATURATES — specificity is min(1, len/2800), so any candidate past ~2800 characters scores exactly 1.000 and all three tie, with the stable sort handing the win to whichever was named first. W483 makes the payload say so (weights_applied {form: 1.0}, the tie is detected and disclosed) instead of advertising weights it never applied, but the RANKING is back to where W419 found it. FIX: an instrument that can separate candidates — or select by an explicit rule and say that is what happened. (found W483 (the §11 screen round) — found while applying the rule, not delivered)
  ...
  P2.9 — The economy's flows told as they happened (virtual WST)
    ...
    FU-264 [medium] [p 17.3] An entity's evolution lifecycle reaches no surface: the cycle stamp, the applied stamp and the apply's own outcome are produced and rendered nowhere — W493's pre-flight keys check names three fields that no page reads. agentic_core/api/vsb.py writes last_evolution_cycle (when a cycle RAN) and last_evolved (when a mutation actually LANDED); no frontend file mentions either, so a user who files ten cycles sees no record that anything happened, and an entity that has never applied a mutation shows nothing distinguishing it from one evolving every day. apply_approved_evolution returns generation_advanced and generation_basis, and no frontend file calls POST /{vsb_id}/evolution/apply at all - the apply is reachable only from the heartbeat (gated on evolution_auto_apply) or a direct API call, so its honest reply about whether the traits changed has no reader. The heartbeat's per-beat record last_vsb_evolved (now carrying the cycle count, the outcome and generation_advanced: False) is published on /api/v1/heartbeat/status and read by no page either. FIX: show the entity's evolution timeline on the cockpit's Organisation or Economy tab - cycles run, last cycle, last APPLIED generation with its date, and whether a review is pending - and give the apply a surface that states generation_basis when an approval is consumed with no applicable mutation. Until then the facts are true and invisible, which is the shape this batch exists to remove. (found W493 refutation (the round's own mechanical pre-flight, keys check))
    ...
    FU-250 [medium] [p 8.8] A stored VSB with no recorded legal form now refuses its economy (422) — nothing migrates the ones already stored — W488 closed a real hole: _resolve_entity_type's vsb-store leg read a top-level 'entity_type' key NO WRITER SETS (the writer records economy.entity_type), so for any VSB known to the vsb store but not the living roster the caller's CLAIM silently became the binding and W313 evaporated. The fix reads the real key and refuses (422) when a stored entity records no form at all — correct, because a claim must not become the binding. But that refusal is now REACHABLE by any VSB stored before the economy block existed: its owner gets a 422 on /economy/status, /waterfall, /cycle, /close-period and /board-pack with no way forward from the page. Needed: a one-pass audit of the stored VSBs for a missing form, a backfill from the living roster where one exists, and a page route that lets the Owner register the form for the rest (the 422 names the entity-types list but no UI reaches it). (found W488 refutation (form-disclosure lens))
...
PHASE P3 — CAPABILITY (Tier 3; ~11 rounds; OWNER RULINGS put with evidence at P3 start)
 ...
 P3.18 [support · docs/AUTONOMOUS_SUPPORT_REVIEW.md] Autonomous technical support, told truthfully. A third
...
scripts/selfcheck_diff.py` — added W493 from the labelled set of 98 defects the W491+W492
refutations confirmed; it finds the shapes that are deterministic, so the fleet spends its budget
on judgement: a key no consumer reads, a key removed while readers survive, a route decorator
bound to a private helper, sibling returns with different key sets, an assertion matching its own
text, a comment quoting a literal its own guard forbids) → REFUTE YOUR OWN FIXES (adversarial
