...
> LATENT entries) · `FABRICATION_LEDGER.md` (closed, 63/63) · `AUTONOMOUS_PROGRESS.md` (W1→W462) ·
...
The execution log runs W1→W462 (462 is the highest NUMBER, not a count; heading-format enumeration
...
W448–W462: the refuters' 61 catches on the regeneration applied and the log completed (W448); then
...
Open 13 (10 scheduled, 1 high · 3 awaiting the Owner) · done 1 · dropped 0.
  NEXT — its own round, before P1.13
    FU-002 [high] A blocked transfer can re-approve an approval an EARLIER action already spent — _materiality_gate returns None both below the threshold (nothing consumed) and after consuming an approval, and the blocked-transfer paths call _restore_consumed_approval unconditionally, which re-approves the latest implemented record with the matching title — so a blocked non-material transfer can hand the next material transfer a spent approval (found W462 register review (read, not yet reproduced))
...
- integration_tests/test_mvp_spine.py — 384 tests; 58 session guards W419–W462, each broken and
