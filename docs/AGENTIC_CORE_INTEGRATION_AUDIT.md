...
- **agentic_core/statistics/live_rigor_monitor** — ✅ REAL (scipy 95% t-interval CI + one-sample t-test; scipy only, NO statsmodels). W437 corrected this entry and the code: the previously certified "power-gated significance" was n/100+0.5 — a call counter, no power analysis existed — and was DELETED; unmeasured results are now null with the reason. INTEGRATED: `POST /api/v1/native-ai/rigor`; logs each validation to the owned UEG chain. Also FIXED a real ledger bug found here: UEG `log_event` crashed on numpy scalars/bools (scipy outputs) → added a numpy-safe json serialiser so the chain never breaks on real payloads.
...
- **agentic_core/signaling/empirical_transduction.EmpiricalSignalTransduction** — ✅ REAL Hill saturation transform `x^h/(K50^h+x^h)` (W437 corrected: the previously certified "pulsatile decoding" and "latency" did NOT exist — the sine reduced to one constant scale factor and latency was 45/(1+x), nothing timed; both were removed rather than renamed). INTEGRATED: `POST /api/v1/native-ai/transduce` + the workflow tree's `signal_response` (supra_threshold means the consensus proceed_fraction ≥ K50, stated as such with basis).
...
- **agentic_core/crypto/entropy_pool.EntropyPool** — ✅ REAL (SHA3-512 + XOR mixing → deterministic 64-bit seed). W437 corrected this entry and the code: the previously certified "pool-integrity digest" was byte-identical to the seed (it could never disagree with what it attested) and was replaced by a disjoint pool-digest slice; the response now states no system entropy is read and the seed must never be a key/nonce/token. INTEGRATED: `POST /api/v1/native-ai/entropy` (reproducible in-house seeding; same sources ⇒ same seed).
