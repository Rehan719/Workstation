...
- ✅ **§13 D1 increment 2 — integrated Website generator.** `POST /api/v1/vsb/{id}/website` generates a real multi-page static HTML/CSS site (index·about·solution + styles) from the entity + in-house copy, into the repo's `web/`, QMS-gated + §11-compliance-screened + document-controlled; served viewable at `…/website/page/{name}` (known pages only). A **Generate integrated Website** action on the Genesis page renders it + an Open-the-live-site link. (Increments 3 Web app + 4 Phone app remain.)
