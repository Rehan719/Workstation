...
A dependency-free i18n layer (`lib/i18n.tsx`: keyed strings, English fallback for anything missing, RTL for ar/ur, language follows `userPrefs.language`) is now built and proven on the **front door** (`/`), translated into **English · Arabic (RTL) · French · Spanish · Urdu (RTL)** — switches live from the Settings language selector. Coverage rolls out surface-by-surface (nav + high-traffic pages next); untranslated strings fall back to English gracefully (honest — never blank/keys). This is genuinely in-house (no LLM needed for the static UI chrome; human/curated translations).
