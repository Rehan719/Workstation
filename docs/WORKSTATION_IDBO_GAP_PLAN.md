...
| **E7** ✅ **DONE (language pref + multilingual voice) / honestly-scoped (AI-output + full-UI translation) (W149)** | **i18n / all languages** (§9) | English-only | DONE (real, today) — a **Language preference** (12 languages, BCP-47) in Settings + `userPrefs.language`; it drives the **DictateButton recognition language**, so users can **dictate in their own language now** (Arabic, Urdu, French, …) via the browser Web Speech API. **Honestly scoped (NOT faked):** AI *text responses* in the chosen language + **full interface-string translation** require the external AI accelerant (a real LLM) — the in-house native floor reasons in English and (verified) does NOT actually translate (the /qep/translation/translate endpoint returns English scaffolding on the native floor). Settings states this transparently. Verified: language select saves "ar-SA"; dictation uses it; honest note shown. |
