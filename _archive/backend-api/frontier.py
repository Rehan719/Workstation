# ARCHIVED (W441) — off-vision, do NOT re-mount as-is.
# This router served five pages archived to _archive/frontend-pages as "off-vision … not part
# of the Workstation IDBO vision" (App.tsx ruling); W441 extended that ruling to the backend.
# The W441 audit found live defects that MUST be fixed before any part is ever resurrected:
#   - reality_grant: a parallel money surface — unbounded/NaN-accepting amounts, no owner stamp,
#     no economy-ledger integration, "allocated" from nothing (capital figures fabricated).
#   - _load/_save: no store_lock / atomic_write_json / load_json_tolerant — the shared-store
#     concurrency class; corruption silently wipes the store on next write.
#   - arvr_session: fabricated render_target "wss://xr.workstation.local/…" (nothing serves it);
#     sessions born "active" with no end/TTL, then re-served as IoT "devices" forever.
#   - cosmic_analyze: gateway.query (no provenance) + exception text returned AS the analysis
#     with status "analyzed"; cosmic_coherence is an uptime ramp, not a measurement.
#   - fires organic-looking sources into the organism nervous feed, bypassing the "manual:"
#     injection marking that /organism/nervous/stimulate deliberately applies.
# Full findings: docs/AUTONOMOUS_PROGRESS.md (W441). The honest in-vision AR/VR surface
# (QEPReligionHub ARVRLab) deliberately consumes nothing until a real WebXR renderer exists.
"""
