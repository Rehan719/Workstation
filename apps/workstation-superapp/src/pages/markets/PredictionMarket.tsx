        ...
        {markets.map(m => (
          <div key={m.id} className="p-8 glass-card border-white/5 hover:border-highlight/30 transition-all flex flex-col gap-6">
             ...
             <div className="grid grid-cols-2 gap-4">
                <button onClick={() => notImplemented('Buy YES')} className="py-4 bg-highlight text-sovereign font-black rounded-xl hover:scale-[1.02] transition-all uppercase tracking-widest text-xs">Buy YES</button>
                <button onClick={() => notImplemented('Buy NO')} className="py-4 bg-white/5 border border-white/10 text-white font-black rounded-xl hover:bg-white/10 transition-all uppercase tracking-widest text-xs">Buy NO</button>
             </div>
